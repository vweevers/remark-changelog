...
class Group extends Section {
  ...
  createList (changes) {
    const list = u('list', { ordered: false, spread: false }, changes.map(change => {
      const children = [u('text', change.title)]
      ...
      if (change.description) {
        ...
        if (isSquashedCommitList(lines)) {
          ...
          // Add full description for context (requires manual intervention)
          children.push(u('code', change.description))
          spread = true
        }
      }
...
function createSublist (lines) {
  ...
  for (const line of lines) {
    ...
    const item = u('listItem', { spread: false }, [u('text', value)])
