...
async function getCommits (opts) {
  ...
  while (!complete && commits.length < limit) {
    ...
    const chunk = await git.log({ dir, gitdir, depth, ref: position })
...
async function tryGetRepo (dir, gitdir) {
  try {
    const remotes = await git.listRemotes({ dir, gitdir })
...
async function resolveCommitish (dir, gitdir, ref) {
  const oid = await git.resolveRef({ dir, gitdir, ref })
  const obj = await git.readObject({ dir, gitdir, oid, encoding: 'utf8' })
