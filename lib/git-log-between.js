...
async function resolveCommitish (dir, ref) {
  let oid = await git.resolveRef({ dir, ref })

  if (ref !== 'head') {
    // Get tagged commit
    const tag = await git.readObject({ dir, oid, encoding: 'utf8' })
    const commit = tag.object

    oid = commit.object

    if (commit.type !== 'commit' || !oid) {
      throw new Error('Could not resolve commitish ' + ref + ' to commit')
    }
  }
